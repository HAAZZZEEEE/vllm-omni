...
"""Read-only validation of msModelSlim single-scale Wan MXFP4 exports.
...
NATIVE_EXPERT_TO_COMPONENT = {"high_noise_model": "transformer", "low_noise_model": "transformer_2"}
...
def _to_diffusers_tensor_name(key: str) -> str:
...
def to_runtime_layer_names(names: set[str]) -> list[str]:
    result = set()
    for name in names:
...
class NativeMXFP4Expert:
    def __init__(self, quant_dir: pathlib.Path, original_dir: pathlib.Path, require_smooth: bool):
        ...
        # AscendV1 writes these objects even without optional transforms. Nonempty
        # values can require rotations or external tensors we do not implement.
        for name in ("metadata", "optional"):
            if name in description:
                value = description.pop(name)
                if not isinstance(value, dict) or value:
                    raise ValueError(f"单级 MXFP4 仅支持空对象 {name}: {value!r}")
                self.source_metadata[name] = value
        ...
        self.native_name_by_diffusers_name: dict[str, str] = {}
        self.quant_layers: set[str] = set()
        for name, label in description.items():
            target = _to_diffusers_tensor_name(name)
            if target in self.native_name_by_diffusers_name:
                raise ValueError(f"重命名冲突: {name} 与 {self.native_name_by_diffusers_name[target]} -> {target}")
            self.native_name_by_diffusers_name[target] = name
            if label == QUANT_TYPE:
        ...
        for runtime_layer in to_runtime_layer_names(self.quant_layers):
        ...
        self._validate_qkv()
        self.ignored_layers = to_runtime_layer_names(
            {
    ...
    def _validate_layers(self, require_smooth: bool) -> None:
        for layer in sorted(self.quant_layers):
            target = layer + ".weight"
            name = self.native_name_by_diffusers_name[target]
    ...
    def _validate_mapping(self) -> None:
        for target, name in self.native_name_by_diffusers_name.items():
        ...
        for target, original in self.original.tensors.items():
            ...
            if (
                target.endswith(".weight")
                and len(original.shape) == 2
                and target not in self.native_name_by_diffusers_name
            ):
    ...
    def _validate_qkv(self) -> None:
        ...
        for group in sorted(groups):
            ...
            if any(layer + ".weight" not in self.native_name_by_diffusers_name for layer in layers):
    ...
    def weights(self) -> Iterator[tuple[str, torch.Tensor]]:
        ...
        for target in sorted(self.original.tensors.keys() | self.native_name_by_diffusers_name.keys()):
            source = self.native_name_by_diffusers_name.get(target)
...
def validate_native_root(original_root: pathlib.Path, quant_root: pathlib.Path) -> None:
    ...
    if model_index.get("_class_name") != "WanPipeline" or any(
        name not in model_index for name in NATIVE_EXPERT_TO_COMPONENT.values()
    ):
    ...
    if (
        directories != set(NATIVE_EXPERT_TO_COMPONENT)
