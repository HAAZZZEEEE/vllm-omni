...
NATIVE_EXPERT_TO_COMPONENT = _native.NATIVE_EXPERT_TO_COMPONENT
...
class ExpertConversion(NativeMXFP4Expert):
    def write(self, directory: pathlib.Path, max_shard_bytes: int) -> dict:
        ...
        self.config["quantization_config"] = {
            ...
            "mxfp4_scale_alg": self.mxfp4_scale_alg,
        }
        _write_json(directory / "config.json", self.config)
        _write_json(
            directory / "quant_model_description.json",
            {target: self.description[source] for target, source in self.native_name_by_diffusers_name.items()},
        )
        return {
...
def repack(
    ...
    *,
    ...
    # 固定配方显式选择 A4 的 UOS 标度算法；本工具不提供其它取值，也不把它
    # 当作无来源的全局默认：调用方必须显式传入，取值写入两位专家的 config
    # 并记入转换报告，便于与生成配方逐条对账。
    if mxfp4_scale_alg != 2:
        raise ValueError("固定单级配方要求 mxfp4_scale_alg=2 (UOS)")
    ...
    experts = {}
    for source, destination in NATIVE_EXPERT_TO_COMPONENT.items():
        expert = ExpertConversion(quant_path / source, original_model_path / destination, require_smooth_scale)
        expert.mxfp4_scale_alg = mxfp4_scale_alg
    ...
    report: dict[str, Any] = {
        ...
        "runtime_scale_alg": {
            ...
            "provenance": "固定单级配方：A4 显式 UOS；写入两位专家的 quantization_config",
        },
        "experts": {},
    }
    ...
    with tempfile.TemporaryDirectory(prefix=".mxfp4-import-", dir=output_path.parent) as temporary:
        ...
        def ignore_experts(directory, names):
            return (
                set(names) & set(NATIVE_EXPERT_TO_COMPONENT.values())
                if pathlib.Path(directory) == original_model_path
                else set()
            )
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        choices=[2],
        default=2,
        help="A4 标度算法；固定单级配方为 2 (UOS)，写入两位专家 quantization_config",
    )
    ...
    try:
        report = repack(
            ...
            mxfp4_scale_alg=args.mxfp4_scale_alg,
