...
NATIVE_MODEL=/path/to/Wan2.2-T2V-A14B

# 配套的原始 BF16 Diffusers 模型（转换输入；必须含 transformer/ 与 transformer_2/）
BF16_DIFFUSERS_MODEL=/path/to/Wan2.2-T2V-A14B-Diffusers-BF16
...
BF16_DIFFUSERS_COMPONENT_ROOT=/path/to/shared-model-components

# 校准输入目录，容器内固定挂载为 /dataset；配方读取 /dataset/index.jsonl
CALIB_DATASET_DIR=/path/to/calibration/wan2_2_t2v
...
MSS_MODELSLIM_LIB=/path/to/msmodelslim-site

# Wan 源码（--trust_remote_code 需要），同时作为容器工作目录
WAN_SRC=/path/to/Wan2.2
...
# 验证过的副本（SHA256 见下一行）：
#   /path/to/dependencies/mindiesd-3.1.0-cp312-cp312-manylinux_2_34_x86_64.whl
MINDIESD_WHEEL=/path/to/dependencies/mindiesd-3.1.0-cp312-cp312-manylinux_2_34_x86_64.whl
